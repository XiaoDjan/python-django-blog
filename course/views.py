...
from .forms import CreateCourseForm, CreatLessonForm
import json
...
class CreatCourseView(UserCourseMixin, CreateView):
...
class CreatLessonView(LoginRequiredMixin, View):
    model = Lesson
    login_url = "/account/login/"

    def get(self, request, *args, **kwargs):
        form = CreatLessonForm(user=self.request.user)
        return render(request, "course/manage/create_lesson.html", {"form": form})

    def post(self, request, *args, **kwargs):
        form = CreatLessonForm(self.request.user, request.POST, request.FILES)
        if form.is_valid():
            new_leeson = form.save(commit=False)
            new_leeson.user = self.request.user
            new_leeson.save()
            return redirect("course:manage_course")
...
class StudentListleLessonView(ListLessonsView):
